targets_import <- list(
    ...
    tar_target(
        pib_excel_file,
        "data-raw/ibge_pib/ibge_pib.xlsx",
        format = "file"
    ),
    
    tar_target(
        raw_ibge_pib_df,
        read_excel(pib_excel_file, skip = 3, n_max = 27)
    ),
    ...
    tar_target(
        renaest_path,
        list.files("data-raw/renaest", pattern = "2024", full.names = TRUE)
    ),
    ...
    # import_snt.R
    tar_target(df_snt, read_snt("https://www.gov.br/transportes/pt-br/assuntos/transito/conteudo-Senatran/municipalizacao-senatran"), cue = tar_cue(mode = "always")),
    tar_target(
        municipios_file,
        "data-raw/snt/ibge_municipios.csv",
        format = "file"
    ),
    ...
    tar_target(
        raw_municipios, 
        read_csv(municipios_file, skip = 6)
    ),
